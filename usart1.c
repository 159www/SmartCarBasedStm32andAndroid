/**
  ...
  */ 
	//串口一有USB转接电路一般用于调试
	//本应将中断函数放入stm32f10x_it.c中,为提高复用性就将USART1接收中断函数置于本文件	
#include "usart1.h"
#include "hc_sr04.h"
#include "BC26.h"
#include "data.h"
//命令响应
//规定命令必须以\r\n结束
char CommandBuffer[100];//命令接收缓冲区
int i1=0;//命令接收缓冲区遍历下标
int ReceiveOneFrameFlag=0;//接收一帧数据标志
int const CommandLength=10; //命令长度
//具体预设命令
...
{                                    
  //uint8_t ucTemp;
	if(USART_GetITStatus(USART1,USART_IT_RXNE)!=RESET)//接收一个字节
	{		
		 USART_ClearITPendingBit(USART1, USART_IT_RXNE);//清空终断标志
		  //将接收数据放入缓冲区
	   if(USART1->DR!='\n')
		 {
		 ...
		 {
			 CommandBuffer[i1++]='\n';
			  if(i1==CommandLength)//过滤掉长度不正确的命令
				{
					...
					if(isEqual(CommandBuffer,Command_Echo,i1))
					{
						...
						//printf("<\r\n");
						//printf(">\r\n");
					  Usart1_SendString(USART1,"今天是个上分的好日子啊好日子啊");
		      }
